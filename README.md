...
We select a stratified random sample of isolated non-settlement pixels, which are identified by by overlaying the 1 arcsecond (as) settlement pixel grid on the 15 as VIIRS grid. Candidate 15 as pixels are those that 1) contain zero 1 as settlement pixels; and 2) are not adjacent to any 15 as cells containing settlement cells. We stratify based on the type of land, selecting up to 500 pixels per category. As there are 17 categories, the theoretical maximum is 8,500. In Ghana, this process results in a sample of about 5,300 isolated non-settlement 15 as pixels.
